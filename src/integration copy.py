...
class PoseGraphSLAM:
    def __init__(self) -> None:
        ...
        self.ang_th = 0.05 # take scan if angle change is > 0.175(10 degrees), 0.785 (45 degrees)
        ...
        # if time.time() - self.previous > 3:
            ...
            with self.mutex:       
                ...
                for j in Ho:
                    ...
                    angle_diff = abs(guess_displacement[-1] - zr[-1])
                    x = [guess_displacement[0], guess_displacement[1]]
                    y = [zr[0], zr[1]]
                    # print('Difference: ', euclidean_distance(x, y))
                    # if euclidean_distance(x, y) <= 0.02:# and angle_diff <= 0.05:  #: 
                    Hp.append(j)

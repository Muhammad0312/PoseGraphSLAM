...
def ICP(MatchedScan, CurrentScan, MatchedVp, CurrentVp):
    ...
    source_points = np.hstack((compounded_scans[0], temp_column.reshape(-1, 1)))

    temp_column = np.zeros(compounded_scans[1].shape[0])
    target_points = np.hstack((compounded_scans[1], temp_column.reshape(-1, 1)))
